...
export const createToken = (payload) => {
  try {
    const token = jwt.sign(payload, process.env.TOKEN_SECRET, { expiresIn: String(process.env.TOKEN_LIFE) })
    const refreshToken = jwt.sign(payload, process.env.REFRESH_TOKEN_SECRET, { expiresIn: String(process.env.REFRESH_TOKEN_LIFE) })
