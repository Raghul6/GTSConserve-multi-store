...
export const addon_order = async (user_id, delivery_date, products, address_id) => {
  ...
  try {
    ...
    const product1 = await knex.select('id', 'user_id').from('add_on_orders').where({ user_id: user_id });
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const sum_values = await knex('add_on_order_items').select(knex.raw('sum(quantity * price) as total_price'))
      ...
      const sub_value = await knex('add_on_orders').insert({sub_total:order_list.total_price}).where({user_id: user_id})
