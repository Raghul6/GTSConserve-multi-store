    ...
    try {
      ...
      if (searchKeyword) {
        const search_data_length = await knex.raw(
          `SELECT subscribed_user_details.id FROM subscribed_user_details JOIN users ON users.id = subscribed_user_details.user_id WHERE subscribed_user_details.branch_id = ${admin_id}   subscribed_user_details.subscription_status = "subscribed" AND users.user_unique_id LIKE '%${searchKeyword}%'`
        );
        ...
        if (data_length.length === 0) {
          ...
          return res.redirect("/branch_admin/subscription/subscribed");
        }
      } else {
      ...
      if (data_length.length === 0) {
        loading = false;
        return res.render("branch_admin/subscription/subscribed", {
          data: data_length,
          searchKeyword,
          routes,
        });
      ...
      let {
        ...
        iterator,
        endingLink,
      } = await getPageNumber(req, res, data_length, "subscription/subscribed");
